...
csv_file_train = DATA_DIR + 'table_00.csv'
csv_file_test = DATA_DIR + 'table_77.csv'
...
rpc_deps_dag = build_rpc_deps(DAG_DIR + 'rpc_dependency_actual.csv')
...
def train_model():
  ...
  x_nodes = ['cpu_util', 'mem_util', 'blkio_read', 'blkio_write']
  x_net_nodes = ['netio_read', 'netio_write']

  services, rpcs = make_sorted_nodes(rpc_deps_dag)
  print(services)
  print(rpcs)
  vae = GraphicalCVAE(rpc_deps_dag, services, rpcs, x_nodes, x_net_nodes,
      dataset_train.feature_cols, root_service='service-nginx', pn_sigma=0.1, 
